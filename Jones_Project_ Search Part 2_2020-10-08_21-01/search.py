...
def search(keyword):
    ...
    if keyword == '':
        ...
        for data in article_metadata():
            if keyword in data[4]:
                 article_data.append(data[0:4])
    return article_data
...
def article_length(max_length, metadata):
    ...
    for articles in metadata:
        if articles[3] <= max_length:
            article_max.append(articles)
    return article_max
# # # 3) 
# # #
...
def random_article(index, metadata):
    if metadata == '' or index >= len(metadata):
        return ''
...
def favorite_author(favorite, metadata):
    for article_info in metadata:
        if favorite in article_info:
            return True
    return False
# # # 6) 
# # #
